 ...
 * only. Not for sale.
 *
 * _coder_Velocities_api.c
 *
 * Code generation for function '_coder_Velocities_api'
 *
 */
                               ...
                               emxArray_struct1_T *y)
{
  ...
  for (i = 0; i < sizes[1]; i++) {
    ...
    y_data[i].id = p_emlrt_marshallIn(
    ...
    y_data[i].mass = p_emlrt_marshallIn(
...
void Velocities_api(const mxArray *const prhs[7], int32_T nlhs,
                    const mxArray *plhs[2])
{
  ...
  emxInit_real_T(&st, &Bij, 4, &c_emlrtRTEI);
  Bij->canFreeData = false;
  emlrt_marshallIn(&st, emlrtAlias(prhs[0]), "Bij", Bij);
  emxInit_real_T(&st, &Bi0, 3, &c_emlrtRTEI);
  Bi0->canFreeData = false;
  c_emlrt_marshallIn(&st, emlrtAlias(prhs[1]), "Bi0", Bi0);
  P0 = e_emlrt_marshallIn(&st, emlrtAlias(prhs[2]), "P0");
  emxInit_real_T(&st, &pm, 2, &c_emlrtRTEI);
  pm->canFreeData = false;
  g_emlrt_marshallIn(&st, emlrtAlias(prhs[3]), "pm", pm);
  u0 = i_emlrt_marshallIn(&st, emlrtAlias(prhs[4]), "u0");
  emxInit_real_T(&st, &um, 1, &c_emlrtRTEI);
  um->canFreeData = false;
  k_emlrt_marshallIn(&st, emlrtAlias(prhs[5]), "um", um);
  emxInitStruct_struct0_T(&st, &robot, &c_emlrtRTEI);
  m_emlrt_marshallIn(&st, emlrtAliasP(prhs[6]), "robot", &robot);
  /* Invoke the target function */
  emxInit_real_T(&st, &tL, 2, &c_emlrtRTEI);
  Velocities(&st, Bij, Bi0, *P0, pm, *u0, um, &robot, *t0, tL);
  ...
  plhs[0] = emlrt_marshallOut(*t0);
  if (nlhs > 1) {
    tL->canFreeData = false;
    plhs[1] = b_emlrt_marshallOut(tL);
  }
  emxFree_real_T(&st, &tL);
  emlrtHeapReferenceStackLeaveFcnR2012b(&st);
}

/* End of code generation (_coder_Velocities_api.c) */
