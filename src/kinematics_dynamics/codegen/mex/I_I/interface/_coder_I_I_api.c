 ...
 * _coder_I_I_api.c
 *
 * Code generation for function '_coder_I_I_api'
 *
 */
...
void I_I_api(const mxArray *const prhs[3], int32_T nlhs, const mxArray *plhs[2])
{
  emlrtStack st = {
      NULL, /* site */
      NULL, /* tls */
  ...
  emxInit_real_T(&st, &RL, 3, &c_emlrtRTEI);
  RL->canFreeData = false;
  c_emlrt_marshallIn(&st, emlrtAlias(prhs[1]), "RL", RL);
  emxInitStruct_struct0_T(&st, &robot, &c_emlrtRTEI);
  e_emlrt_marshallIn(&st, emlrtAliasP(prhs[2]), "robot", &robot);
  /* Invoke the target function */
  emxInit_real_T(&st, &Im, 3, &c_emlrtRTEI);
  I_I(&st, *R0, RL, &robot, *I0, Im);
  emxFreeStruct_struct0_T(&st, &robot);
  emxFree_real_T(&st, &RL);
  /* Marshall function outputs */
  plhs[0] = emlrt_marshallOut(*I0);
  if (nlhs > 1) {
    Im->canFreeData = false;
    plhs[1] = b_emlrt_marshallOut(Im);
  }
  emxFree_real_T(&st, &Im);
  emlrtHeapReferenceStackLeaveFcnR2012b(&st);
}

/* End of code generation (_coder_I_I_api.c) */
