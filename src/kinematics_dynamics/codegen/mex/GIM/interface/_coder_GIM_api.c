 ...
 * _coder_GIM_api.c
 *
 * Code generation for function '_coder_GIM_api'
 *
 */
...
void GIM_api(const mxArray *const prhs[7], int32_T nlhs, const mxArray *plhs[3])
{
  emlrtStack st = {
      NULL, /* site */
      NULL, /* tls */
  ...
  H0 = (real_T(*)[36])mxMalloc(sizeof(real_T[36]));
  emlrtHeapReferenceStackEnterFcnR2012b(&st);
  /* Marshall function inputs */
  M0_tilde = emlrt_marshallIn(&st, emlrtAlias(prhs[0]), "M0_tilde");
  emxInit_real_T(&st, &Mm_tilde, 3, &f_emlrtRTEI);
  Mm_tilde->canFreeData = false;
  c_emlrt_marshallIn(&st, emlrtAlias(prhs[1]), "Mm_tilde", Mm_tilde);
  emxInit_real_T(&st, &Bij, 4, &f_emlrtRTEI);
  Bij->canFreeData = false;
  e_emlrt_marshallIn(&st, emlrtAlias(prhs[2]), "Bij", Bij);
  emxInit_real_T(&st, &Bi0, 3, &f_emlrtRTEI);
  Bi0->canFreeData = false;
  c_emlrt_marshallIn(&st, emlrtAlias(prhs[3]), "Bi0", Bi0);
  P0 = emlrt_marshallIn(&st, emlrtAlias(prhs[4]), "P0");
  emxInit_real_T(&st, &pm, 2, &f_emlrtRTEI);
  pm->canFreeData = false;
  g_emlrt_marshallIn(&st, emlrtAlias(prhs[5]), "pm", pm);
  emxInitStruct_struct0_T(&st, &robot, &f_emlrtRTEI);
  i_emlrt_marshallIn(&st, emlrtAliasP(prhs[6]), "robot", &robot);
  /* Invoke the target function */
  emxInit_real_T(&st, &H0m, 2, &f_emlrtRTEI);
  emxInit_real_T(&st, &Hm, 2, &f_emlrtRTEI);
  GIM(&st, *M0_tilde, Mm_tilde, Bij, Bi0, *P0, pm, &robot, *H0, H0m, Hm);
  ...
  plhs[0] = emlrt_marshallOut(*H0);
  if (nlhs > 1) {
    H0m->canFreeData = false;
    plhs[1] = b_emlrt_marshallOut(H0m);
  }
  emxFree_real_T(&st, &H0m);
  if (nlhs > 2) {
    Hm->canFreeData = false;
    plhs[2] = b_emlrt_marshallOut(Hm);
  }
  emxFree_real_T(&st, &Hm);
  emlrtHeapReferenceStackLeaveFcnR2012b(&st);
}

/* End of code generation (_coder_GIM_api.c) */
